...
import { convertPlayerName, convertPosition, getSeasonImg } from '../../../utils/MatchStatistics';
            ...
            .map((i, index) => {
              return (
                <li key={index}>
                  ...
                  <p>
                    드리블 :{' '}
                    {i.status.dribbleTry !== 0 ? Math.floor((i.status.dribbleSuccess / i.status.dribbleTry) * 100) : 0}%
                  ...
                  <p>
                    공중볼 경합 :{' '}
                    {i.status.aerialTry !== 0 ? Math.floor((i.status.aerialSuccess / i.status.aerialTry) * 100) : 0}% (
                    {`${i.status.aerialTry}/${i.status.aerialSuccess}`})
                  </p>
                  <p>
                    슬라이딩 태클 :{' '}
                    {i.status.tackleTry !== 0 ? Math.floor((i.status.tackle / i.status.tackleTry) * 100) : 0}% (
                    {`${i.status.tackleTry}/${i.status.tackle}`})
                  </p>
                  <p>
                    볼 소유 시도 :{' '}
                    {i.status.ballPossesionTry !== 0
                      ? Math.floor((i.status.ballPossesionSuccess / i.status.ballPossesionTry) * 100)
                      : 0}
